...
def render_grid_cards(opps_list, category_name: str):
    ...
    current_page = st.number_input("Page", min_value=1, max_value=max(1, total_pages), value=1, key="page_long_term")
...
def render():
    ...
    # Fetch long-term opportunities from DB (low risk or high TVL filter)
    @st.cache_data(ttl=300)
    def cached_long_term():
        all_opps = db.get_opportunities(limit=100)
        return [
            opp for opp in all_opps
            if safe_get(opp, 'risk', 'Unknown') == 'Low' or float(safe_get(opp, 'tvl', 0.0)) > 1_000_000
        ]
