...
def generate_pdf(scan_results: Dict[str, Any], filename: str = "defi_report.pdf") -> None:
    ...
    for category, items in scan_results.items():
        ...
        for i, item in enumerate(items):
            if category == "memes":
                line = (
                    f"{i+1}. {item['chain']} | {item['symbol']} | Price: {item['price_usd']} | "
                    f"Liq: {item['liquidity_usd']} | Vol24h: {item['volume_24h_usd']} | "
                    f"Change24h: {item['change_24h_pct']} | Risk: {item['risk']} | {item['url']}"
                )
            else:
                line = (
                    f"{i+1}. {item['chain']} | {item['project']} | {item['symbol']} | "
                    f"APY: {item['apy_str']} | ROR: {item['ror']:.2f} | TVL: {item['tvl_str']} | "
                    f"Risk: {item['risk']} | Gas: {item['gas_fee_str']} | {item['link']}"
                )
            # Wrap long lines
            if len(line) > 100:
...
# ---------- Async Yield Entry Fetchers ----------
async def get_top_picks(limit: int = 10) -> List[YieldEntry]:
    try:
...
async def get_short_term_opportunities(limit: int = 5) -> List[YieldEntry]:
    try:
        entries = await fetch_yields()
        if entries:
